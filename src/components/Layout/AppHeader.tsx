...
const AppHeader = ({ currentScreen, onNavigate }: AppHeaderProps) => {
  return (
    <header className="px-4 md:px-6 lg:px-8 py-3 text-white" style={{ background: 'linear-gradient(135deg, #68C72A 0%, #029100 100%)' }}>
      <div className="flex items-center justify-between">
        <div className="flex items-start">
          <SlideOutMenu onNavigate={onNavigate}>
        ...
        <div className="grid grid-cols-2 gap-1 md:gap-2">
          <Button 
            size="sm" 
            className={`bg-white hover:bg-gray-50 font-roboto text-sm md:text-base font-bold px-2 md:px-3 py-1 w-12 h-12 md:w-16 md:h-16 lg:w-20 lg:h-20 border-2 ${currentScreen === 'main' ? '' : 'opacity-50'}`}
            style={{ color: '#015D00', borderColor: '#015D00' }}
          ...
          <Button 
            size="sm" 
            className={`bg-white hover:bg-gray-50 font-roboto text-sm md:text-base font-bold px-2 md:px-3 py-1 w-12 h-12 md:w-16 md:h-16 lg:w-20 lg:h-20 border ${currentScreen === 'add' ? 'border-2' : 'opacity-50'}`}
            style={{ color: '#015D00', borderColor: '#015D00' }}
          ...
          <Button 
            size="sm" 
            className={`bg-white hover:bg-gray-50 font-roboto text-sm md:text-base font-bold px-2 md:px-3 py-1 w-12 h-12 md:w-16 md:h-16 lg:w-20 lg:h-20 border ${currentScreen === 'grow' ? 'border-2' : 'opacity-50'}`}
            style={{ color: '#015D00', borderColor: '#015D00' }}
          ...
          <Button 
            size="sm" 
            className={`bg-white hover:bg-gray-50 font-roboto text-sm md:text-base font-bold px-2 md:px-3 py-1 w-12 h-12 md:w-16 md:h-16 lg:w-20 lg:h-20 border ${currentScreen === 'how' ? 'border-2' : 'opacity-50'}`}
            style={{ color: '#015D00', borderColor: '#015D00' }}
